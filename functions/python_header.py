...
How it works:
  1. Loads auxiliary *.env files from the calling script's directory without overriding the process env
  2. Loads .env from the calling script's directory last, overriding matching injected values
  3. Precedence is .env > injected process env > auxiliary *.env defaults
...
# Load auxiliary env files first as defaults, then .env last as the local override.
_env_dir = _find_env_file().parent
_dot_env = _env_dir / ".env"
for _ef in sorted(p for p in _env_dir.glob("*.env") if p.name != ".env"):
